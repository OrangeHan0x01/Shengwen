...
__all__ = ['ASRExecutor','EasrExecutor']
...
class EasrExecutor(BaseExecutor):
    def __init__(self):
        super(EasrExecutor, self).__init__()
        self.change_format = False
        self.sample_rate=16000
        self.input=''#这里用的时候改成输入的文件名
        self.model_n='conformer_wenetspeech'
        self.lang='zh'
        self.config=None
        self.decode_method='attention_rescoring'
        #self.cfg_path:'model.yaml'
        self.ckpt_path=None
        self.yes=False
...
#将model加载和识别分开！
    ...
    def init_model(self):#去除了check，加快运行
        paddle.set_device(self.device)
        self._init_from_path(self.model, self.lang, self.sample_rate, self.config, self.decode_method,self.ckpt_path)

    def exe_audio_file(self,audio_file):
        self.preprocess(self.model, audio_file)
